/* ==========================================================================
   ...
   SSS, sektörler, ulaşım seçenekleri ve paydaşlar tek sayfada sekmeli olarak
   yönetilir. Dördü de "kısa kayıt listesi + TR/EN alanlar + sıra" biçiminde
...
const SEKMELER = [
  {
    ...
    yayinVar: true,
    alanlar: [
      { ad: 'slug', etiket: 'Slug', ipucu: 'Küçük harf, rakam ve tire. Örn: bilet-fiyati' },
  ...
  },
  {
    ...
    alanlar: [
      ...
      { ad: 'ikon', etiket: 'İkon dosyası', ipucu: 'assets/icons/ içindeki dosya adı' }
    ]
  },
  {
  ...
  },
  {
    ...
    alanlar: [
      ...
      { ad: 'seviye', etiket: 'Seviye',
        secenekler: ['partner', 'ana', 'altin', 'gumus', 'destekci'],
        ipucu: 'partner = kurucu paydaş' },
      { ad: 'logo_url', etiket: 'Logo yolu' },
      { ad: 'web', etiket: 'Web sitesi' }
    ]
  }
];
  ...
  el('div', { class: 'sayfa-bas' },
    el('div', { class: 'sayfa-bas__metin' },
      ...
      el('p', { class: 'sayfa-bas__alt' },
        'Sık sorulan sorular, sektörler, ulaşım bilgileri ve paydaşlar.')))
);
...
async function yukle() {
  ...
  const s = aktifSekme;
  const { data, error } = await sb.from(s.tablo).select(s.secim).order('sira');
  ...
  if (!data.length) {
    panel.replaceChildren(bas, durumKutusu({
  ...
  const govde = el('tbody', null, data.map((r, i) => {
    const tr = el('tr', { tabindex: '0', role: 'button' },
      el('td', { class: 'tablo__no' }, String(r.sira ?? i)),
  ...
  panel.replaceChildren(bas,
    el('div', { class: 'tablo-sarmal' },
      el('table', { class: 'tablo' },
        el('thead', null, el('tr', null,
          el('th', null, 'Sıra'), el('th', null, 'Başlık'),
...
function duzenle(r) {
  ...
  const alanlar = s.alanlar.map((a) => {
    ...
    if (a.secenekler) {
      g = el('select', { class: 'form__control', id },
        a.secenekler.map((o) => el('option', { value: o, selected: kayit[a.ad] === o }, o)));
    } else if (a.cokSatir) {
    ...
    return el('div', { class: 'form__group' },
      el('label', { class: 'form__label', for: id }, a.etiket), g,
  ...
  kaydetBtn.addEventListener('click', async () => {
    ...
    if (!baslik) {
      toast(`${s.alanlar.find((a) => a.ad === s.baslikAlan).etiket} boş olamaz.`, 'hata');
      giris[s.baslikAlan].eleman.focus();
      return;
    }
